...
describe 'mongo::default' do
    context 'When all attributes are default, on Ubuntu 16.04' do
      ...
      it "should create the mongod.conf file in /etc/mongod.conf" do
        expect(chef_run).to create_template("/etc/mongod.conf")
      end

      it "should create the mongod.service file in /lib/systemd/system/mongod.service" do
        expect(chef_run).to create_template("/lib/systemd/system/mongod.service")
      end

      it "mongod service should be enabled" do
        expect(chef_run).to enable_service("mongod")
      end

      it "mongod service should be started" do
